        ...
        for number in info.lobby.game[creatorid][team][mob]:
         if info.lobby.game[creatorid][team][mob][number]['smert']!=1:
           if info.lobby.game[creatorid][team][mob][number]['target']==None:
              ...
              if team=='t1mobs':
               if t['hp']<1:
                info.lobby.game[creatorid]['resultst1']+=info.lobby.game[creatorid][team][mob][number]['name']+'('+typetotext(info.lobby.game[creatorid][team][mob][number]['type'])+')'+' нанёс '+str(dmg)+' урона по '+t['name']+'('+typetotext(t['type'])+')'+str(t['x'])+skilltext+'; Враг погибает!'+"\n"  
               else:
                info.lobby.game[creatorid]['resultst1']+=info.lobby.game[creatorid][team][mob][number]['name']+'('+typetotext(info.lobby.game[creatorid][team][mob][number]['type'])+')'+' нанёс '+str(dmg)+' урона по '+t['name']+'('+typetotext(t['type'])+')'+str(t['x'])+skilltext+'; '+'У него остается '+t['hp']+' хп!'+"\n"
              elif team=='t2mobs':
               if t['hp']<1:
                info.lobby.game[creatorid]['resultst2']+=info.lobby.game[creatorid][team][mob][number]['name']+'('+typetotext(info.lobby.game[creatorid][team][mob][number]['type'])+')'+' нанёс '+str(dmg)+' урона по ('+t['name']+typetotext(t['type'])+')'+str(t['x'])+skilltext+'; Враг погибает!'+"\n"
               else:
                info.lobby.game[creatorid]['resultst2']+=info.lobby.game[creatorid][team][mob][number]['name']+'('+typetotext(info.lobby.game[creatorid][team][mob][number]['type'])+')'+' нанёс '+str(dmg)+' урона по ('+t['name']+typetotext(t['type'])+')'+str(t['x'])+skilltext+'; '+'У него остается '+t['hp']+' хп!'+"\n"
